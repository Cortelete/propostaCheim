...
const ValueItem = ({ title, description, value }: { title: string, description: string, value: string }) => (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-left">
        <div>
            <h4 className="font-semibold text-white group-hover:text-amber-300 transition-colors duration-300">{title}</h4>
            <p className="text-sm text-gray-400 mt-1 max-w-lg">{description}</p>
        </div>
        <p className="text-lg font-bold text-gray-300 mt-2 sm:mt-0 sm:ml-4 whitespace-nowrap flex items-center gap-2">
            R$ {value}
...
const MethodologyItem: React.FC<{ icon: React.ReactNode; title: string; description: string; }> = ({ icon, title, description }) => (
    <div className="flex items-start gap-4">
        <div className="bg-amber-400/10 p-3 rounded-lg mt-1 flex-shrink-0">
            {icon}
        </div>
        <div>
...
const ValueBreakdownSection: React.FC<ValueBreakdownSectionProps> = ({ onOpenModal }) => {
      ...
      onOpenModal({
        ...
        content: (
          <div className="space-y-6">
            ...
            <div className="bg-amber-900/50 border border-amber-400/30 rounded-lg p-4 text-center">
              <h4 className="font-bold text-amber-300 text-lg">Atenção!</h4>
            ...
            <p className="text-lg text-white">Valor de mercado deste item: <span className="font-bold">R$ {item.value}</span></p>

            <a
              href={item.href}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block w-full text-center bg-gray-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-gray-500 transition-colors duration-300"
            >
  ...
  return (
    <section id="value-breakdown" className="mt-24 sm:mt-32">
        ...
        <div className="max-w-4xl mx-auto bg-gradient-to-b from-[#1c1c1c] to-[#151515] border-2 border-amber-400/30 rounded-2xl p-8 shadow-2xl shadow-amber-500/10">
            <div className="flex flex-col">
                {valueItemsData.map((item, index) => (
                    <button
                        key={index}
                        onClick={() => handleValueItemClick(item)}
                        className="p-4 group transition-all duration-300 border-b border-gray-800 last-of-type:border-b-0 hover:bg-white/5 hover:rounded-lg w-full"
            ...
            <div className="mt-8 pt-6 border-t border-gray-800 text-center space-y-2 group">
                <p className="text-lg text-gray-400">Valor Total de Mercado:</p>
                <p className="text-3xl font-bold text-red-500 line-through">R$ 11.500,00</p>
            ...
            <div className="mt-8 pt-8 border-t border-gray-800/50 text-center">
                <button
                    ...
                    className="text-amber-300 font-semibold hover:text-white transition-colors duration-300 group"
